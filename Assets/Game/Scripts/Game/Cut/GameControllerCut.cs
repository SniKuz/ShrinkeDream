...
namespace Game.Cut
{
    public class GameControllerCut : GameController
    {
        ...
        private void Start()
        {
            foreach (CutContainer container in CutContainer)
            {
            ...
            {
                obj.TryGetComponent(out CanvasGroup cag);
                if (cag)
                {
                    cag.alpha = 0;
                }
            ...
            currentCutIndex = 0;
            CutContainer[currentContainer].gameObject.SetActive(true);
        ...
        private IEnumerator NextCut()
        {
            ...
            CutContainer[currentContainer].cutObjects[currentCutIndex].TryGetComponent(out CanvasGroup cag);
            if (cag)
            {
                sequence.Append(cag.DOFade(1, fadeTime))
                .Play();
            }
            else
            {
